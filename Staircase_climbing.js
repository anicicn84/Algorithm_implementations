...
function staircaseTraversal(height, maxSteps) {
  // sliding window solution
	...
	const waysToTop = [1];
	// <= height is to make sure we are including the hight
	for (let currentHeight = 1; currentHeight <= height; currentHeight++) {
		// we need to calculate start & end of our window
		// start of the prev window so that we can substract that value
		...
		if (startOfPreviousWindow >= 0) { // it is not in the negative, left of 0
			currentNumberOfWays -= waysToTop[startOfPreviousWindow]; // remove the element from the left
																															// which got dropped out of the window
